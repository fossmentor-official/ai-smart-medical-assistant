    ...
    <div className="flex flex-col bg-slate-50" style={{ height: "100vh" }}>
      ...
      {!available && (
        <div className="flex-shrink-0 mx-4 mb-1 flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-amber-50 border border-amber-200">
          ...
          <div className="flex-1 min-w-0">
            <span className="text-[12px] font-semibold text-amber-700">Usage limit reached</span>
            <span className="text-[12px] text-amber-600 ml-1.5">Clinical chatbot resets in {countdown}</span>
          </div>
          <span className="text-[10px] text-amber-500 font-medium border border-amber-300 rounded-full px-2 py-0.5">1×/hr</span>
        </div>
      )}
      ...
      <div className="border-t border-slate-200 bg-white px-4 py-3 flex-shrink-0">
        <div className="max-w-4xl mx-auto flex items-end gap-3">
          <div className={`flex-1 flex items-end gap-2 rounded-2xl border bg-slate-50 px-4 py-2.5 transition-all ${
            overLimit
            ...
            {/* Char counter */}
            <div className={`text-[11px] font-mono font-semibold flex-shrink-0 self-end pb-0.5 transition-colors ${charColor} ${charBg && `px-1.5 py-0.5 rounded border ${charBg}`}`}>
              {charCount}/{MAX_CHARS}
            </div>
          </div>
